...
function CCSUILoader:createUINode(clsName, options, parent)
	...
	elseif clsName == "Panel" then
		node = self:createPanel(options, parent)
...
function CCSUILoader:createPanel(options)
	...
	--
    if parent ~= nil 
        and options.sizeType == 1 
        and options.clipAble 
        and options.x == 0 
        and options.y == 0 then

        local parentSize = parent:getContentSize()
        local x = parentSize.width * options.sizePercentX
        local y = parentSize.height * options.sizePercentX
        if x == display.width and y == display.height then
            options.adaptScreen = true
            --TODO("CCSUILoader:createPanel()", options.name, options.x, options.y, options.width, options.height)
        end
    end
